...
object Ut {
    ...
    object file {
        ...
        fun downloadByHttp(url: String, dirPath: String, uniqueFilename: Boolean): String {
            ...
            filename =
                if (uniqueFilename) "${UUID.randomUUID()}$ORIGINAL_FILE_NAME_SEPARATOR$filename"
                else filename
        ...
        @JvmStatic
        fun getFilenameWithoutExtFromUrl(url: String): String {
            try {
                val path = URI(url).getPath();
        ...
        @JvmStatic
        fun getExtensionFromResponse(response: HttpResponse<*>): String {
        ...
        fun getMetadata(filePath: String): Map<String, Any> {
            ...
            return if (fileExtTypeCode == "img") getImgMetadata(filePath)
            else emptyMap()
        }
        ...
        fun toFile(multipartFile: MultipartFile, dirPath: String, metaStr: String): String {
            ...
            val fileName =
                if (str.isBlank(metaStr)) "${UUID.randomUUID()}$ORIGINAL_FILE_NAME_SEPARATOR${multipartFile.originalFilename}"
                else "$metaStr$META_STR_SEPARATOR${UUID.randomUUID()}$ORIGINAL_FILE_NAME_SEPARATOR${multipartFile.originalFilename}"
        ...
        fun withNewExt(fileName: String, fileExt: String): String {
            return if (fileName.contains(".")) fileName.substring(0, fileName.lastIndexOf('.') + 1) + fileExt
        ...
        fun getMetadataStrFromFileName(filePath: String): String {
            ...
            return if (fileName.contains(META_STR_SEPARATOR)) fileName.substring(
                0,
                fileName.indexOf(META_STR_SEPARATOR)
            )
            else ""
        }
    }
